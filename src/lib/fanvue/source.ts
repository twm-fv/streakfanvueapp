...
import { hasScope, SCOPE_CREATOR, SCOPE_INSIGHTS } from "./scopes";
...
const EARNING_DATE_KEYS = ["date", "day", "periodStart", "period_start", "createdAt"];
...
export class FanvueSource implements ActivitySource {
  ...
  constructor(
    ...
    this.client = new FanvueClient(accessToken);
  }
  ...
  async getActivity(historyDays: number, timezone: string): Promise<ActivityWindow> {
    ...
    if (hasScope(this.grantedScopes, SCOPE_CREATOR)) {
      try {
        await this.collectPosts({ start, timezone, counts });
      } catch (error) {
        warnings.push(
          error instanceof FanvueApiError
            ? `Could not read your posts (API returned ${error.status}).`
            : "Could not read your posts.",
        );
      }
    } else {
      warnings.push(
        `Posting history needs the ${SCOPE_CREATOR} scope. Reconnect and grant it to see your streak.`,
      );
    }
    ...
    if (hasScope(this.grantedScopes, SCOPE_INSIGHTS)) {
      try {
        ...
        warnings.push(
          error instanceof FanvueApiError
            ? `Could not read earnings insights (API returned ${error.status}).`
            : "Could not read earnings insights.",
        );
      }
    }
  ...
  private async collectEarnings(start: string, end: string): Promise<Map<string, number>> {
    const payload = await this.client.get<unknown>(env.API_INSIGHTS_EARNINGS_PATH, {
      startDate: start,
      endDate: end,
      interval: "day",
    });
